...
def search_events(dmsp_file, omniweb_file, outfolder=None, no_plots=False):
    ...
    """
    # Do computation --------------------------------------------------
    dmsp_fh, omniweb_fh = lib_search_dispersion.read_files(dmsp_file, omniweb_file)
    ...
    for _, row_match in df_match.iterrows():
        ...
        im = axes[0].pcolor(dmsp_fh['t'][i:j], np.log10(dmsp_fh['ch_energy']), dmsp_fh['ion_d_ener'][:, i:j], 
        ...
        axes[0].axhline(np.log10(lib_search_dispersion.MAX_ENERGY_ANALYZED), color='black', linestyle='dashed')
        axes[0].invert_yaxis()
        ...
        title += f' -- Integral {float(np.nanmax(integral[i:j])):.2f} -- UpperFrac {float(upper_area_frac[i]):.2f}\n'
        title += f'Bz =  {row_match["Bz_mean"]:.2f} nT'
